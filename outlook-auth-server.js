...
const server = http.createServer((req, res) => {
  ...
  if (pathname === '/auth/callback') {
    ...
    const stateEntry = pendingStates.get(query.state);
    const isCrmLogin = stateEntry && stateEntry.isCrm;
    const crmPort    = (stateEntry && stateEntry.crmPort) || 3000;
    ...
    if (query.code) {
      ...
      exchangeCodeForTokens(query.code)
        .then((tokens) => {
          ...
          if (isCrmLogin) {
            // CRM login: redirect browser back to the CRM server so it can create a session
            res.writeHead(302, { 'Location': `http://localhost:${crmPort}/crm/auth/resume` });
            res.end();
          } else {
    ...
    const isCrm   = parsedUrl.query.crm === '1';
    const crmPort = parseInt(parsedUrl.query.crmPort || '3000', 10);
    pendingStates.set(state, { timestamp: Date.now(), isCrm, crmPort });
