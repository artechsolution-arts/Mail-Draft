...
class TokenStorage {
  constructor(config) {
    ...
    if (!this.config.clientId || !this.config.clientSecret) {
      console.error("TokenStorage: Client ID or Secret is not configured (checked MS_CLIENT_ID/OUTLOOK_CLIENT_ID). Token refresh will fail.");
    }
  }
  ...
  async _saveTokensToFile() {
    if (!this.tokens) {
      console.error('No tokens to save.');
      return false;
    }
    try {
